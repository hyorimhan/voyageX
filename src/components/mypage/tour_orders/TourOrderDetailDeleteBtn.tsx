...
import { deleteTourOrderDetail } from '@/services/tour';
...
const TourOrderDetailDeleteBtn = ({ order_id }: TourOrderDetailListProps) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const router = useRouter();

  const handleDeleteTourOrder = async () => {
    try {
      await deleteTourOrderDetail(order_id);
    } catch (error) {
  ...
  const confirmDelete = () => {
    ...
    router.push('/mypage/tour_orders');
  };
...
export default TourOrderDetailDeleteBtn;
