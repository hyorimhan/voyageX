  ...
  <div className='scroll-container h-full w-full relative flex items-center justify-center'>
    ...
    <div className='slider-container relative flex items-center justify-center'>
      {planets.map((planet, index) => {
        ...
        const isActive =
          index ===
          (currentSlide + Math.floor(visiblePlanetsCount / 2)) % planets.length;
...
>
  <h1
    className={`text-4xl absolute font-semibold top-40 left-20 ${orbitron.className} transition-opacity duration-500 ${videoLoaded ? 'opacity-100' : 'opacity-0'}
    sm:text-2xl sm:font-medium sm:top-16 sm:left-4`}
  >
    GOODS SHOP
  </h1>
  <Link href='/shop'>
    <p className={`absolute top-48 right-20 underline transition-opacity duration-500 ${videoLoaded ? 'opacity-100' : 'opacity-0'} sm:top-16 sm:right-4`}>
      MORE+
    </p>
  </Link>
